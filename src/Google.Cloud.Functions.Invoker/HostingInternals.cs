...
namespace Google.Cloud.Functions.Invoker
{
    internal static class HostingInternals
    {
        ...
        internal static void AddCommandLineArguments(IConfigurationBuilder configBuilder, string[] args)
        {
        ...
        internal static void ConfigureApplication(WebHostBuilderContext context, IApplicationBuilder app)
        {
            var configurers = app.ApplicationServices.GetServices<FunctionsApplicationConfigurer>();
        ...
        internal static IEnumerable<FunctionsServiceProvider> GetServiceProviders(Assembly assembly) =>
            assembly
                .GetCustomAttributes<FunctionsServiceProviderAttribute>()
                .OrderBy(attr => attr.Order)
                .ThenBy(attr => attr.ProviderType.FullName)
                .Select(attr => Activator.CreateInstance(attr.ProviderType))
                .Cast<FunctionsServiceProvider>();

        internal static void ConfigureServiceProviders(WebHostBuilderContext context, IServiceCollection services, IEnumerable<FunctionsServiceProvider> providers)
        {
            foreach (var provider in providers)
            {
                provider.ConfigureServices(context, services);
            }
        }

        internal static IEnumerable<FunctionsApplicationConfigurer> GetApplicationConfigurers(Assembly assembly) =>
            assembly
                .GetCustomAttributes<FunctionsApplicationConfigurerAttribute>()
                .OrderBy(attr => attr.Order)
                .ThenBy(attr => attr.ConfigurerType.FullName)
                .Select(attr => Activator.CreateInstance(attr.ConfigurerType))
                .Cast<FunctionsApplicationConfigurer>();

        internal static void ConfigureApplicationConfigurers(WebHostBuilderContext context, IServiceCollection services, IEnumerable<FunctionsApplicationConfigurer> configurers)
        {
            foreach (var configurer in configurers)
            {
        ...
        internal static Type GetFunctionTarget(WebHostBuilderContext context, Assembly assembly)
        {
            ...
            return target is null
                ...
                : assembly.GetType(target) ?? throw new Exception($"Can't load specified function type '{target}'");
        }

        internal static void AddServicesForTarget(IServiceCollection services, Type functionType)
        {
            ...
            {
                throw new Exception($"Function type '{functionType.FullName}' doesn't support any known function interfaces.");
            }
        }
        ...
        private sealed class FunctionTypeProvider
        {
