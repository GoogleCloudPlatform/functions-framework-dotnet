...
```sh
dotnet new -i Google.Cloud.Functions.Templates::1.0.0-alpha04
```
...
`--target` (or only argument) | `FUNCTION_TARGET`         | The name of the target function (implementing `IHttpFunction`, `ICloudEventFunction` or `ILegacyEventFunction<T>`) to be invoked in response to requests.
